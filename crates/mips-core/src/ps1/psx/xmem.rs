...
use crate::ps1::psx::bios::bios::BIOS_SIZE;
...
impl XMemory {
    ...
    pub fn copy_bios(&mut self, source: &XMemory) -> MipsResult<()> {
        ...
        if self.bios_sha256 != source.bios_sha256 && self.bios_sha256.iter().any(|&b| b != 0) {
            return Err(MipsError::from(Ps1Error::UnknownBios("BIOS SHA-256 don't match".to_string())));
        }
    ...
    fn store_block_u8(&mut self, offset: u32, block: &[u8], size: usize) {
        let idx = (offset / 4) as usize;

        let i = 0;
        let num_u32s = size / 4;
        for i in 0..num_u32s {
            let b0 = block[4 * i] as u32;
            let b1 = block[4 * i + 1] as u32;
            let b2 = block[4 * i + 2] as u32;
            let b3 = block[4 * i + 3] as u32;
            self.memory[idx + i] = b0 | (b1 << 8) | (b2 << 16) | (b3 << 24);
        }
    }
