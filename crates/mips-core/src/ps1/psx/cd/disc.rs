...
impl SerialNumber {
    ...
    fn from_bin_name(bin: &[u8]) -> MipsResult<SerialNumber> {
        if bin.len() != 11 {
...
#[allow(dead_code)]
pub fn extract_system_region(image: &mut dyn Image) -> MipsResult<Region> {
    ...
    let toc = image.toc();
    let track = toc.track(Bcd::ONE).unwrap();
    let msf = track.disc_position(Msf::from_bcd(0x00, 0x00, 0x04).unwrap()).unwrap();

    let sector = image.read_sector(msf).unwrap();
    ...
    let license_blob = &sector.mode2_xa_payload().unwrap()[0..76];
    ...
    let region = match license.as_ref() {
        ...
        _ => {
            ...
            return Err(MipsError::from(Ps1Error::BadDiscFormat(m)));
        }
    };
...
fn read_system_cnf(image: &mut CdCache) -> MipsResult<Vec<u8>> {
    ...
    let system_cnf = dir.entry_by_name(b"SYSTEM.CNF;1").unwrap();
    ...
    if len > 1024 * 1024 {
        ...
        return Err(MipsError::from(Ps1Error::BadDiscFormat(desc)));
    }

    let system_cnf = system_cnf.read_file(image).unwrap();
