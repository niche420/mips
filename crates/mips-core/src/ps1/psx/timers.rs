...
impl Timers {
    ...
    fn next_irq(&self) -> Option<ClockCycle> {
        let mut delta = None;
    ...
    fn run_cpu(&mut self, which: usize, cpu_cycles: ClockCycle) -> bool {
        let source = self.clock_source(which);
    ...
    fn run_gpu_clock(&mut self, which: usize, gpu_cycles: ClockCycle) -> bool {
        let source = self.clock_source(which);
    ...
    fn run_gpu_hsync(&mut self, which: usize, hsync_cycles: ClockCycle) -> bool {
        let source = self.clock_source(which);
...
impl Timer {
    ...
    fn write_counter(&mut self, val: u16) {
        self.counter = ClockCycle::from(val);
    ...
    fn target_match(&self) -> bool {
        ClockCycle::from(self.target) == self.counter
    }
    ...
    fn set_match(&mut self) -> bool {
        ...
        if self.mode.reset_counter_on_target() {
            if self.target == 0 {
                ...
                self.counter %= ClockCycle::from(self.target);
            }
        }
    ...
    /// predicted)
    fn next_irq(&self, source: Clock, sync_mode: SyncMode) -> Option<ClockCycle> {
        ...
        let target = ClockCycle::from(self.target);
    ...
    fn run_cpu(&mut self, source: Clock, cpu_cycles: ClockCycle) -> bool {
    ...
    fn run_gpu_clock(&mut self, source: Clock, gpu_cycles: ClockCycle) -> bool {
        if source == Clock::GpuPixClk {
            self.run(gpu_cycles)
        } else {
    ...
    fn run_gpu_hsync(&mut self, source: Clock, hsync_cycles: ClockCycle) -> bool {
        if source == Clock::GpuHSync {
...
fn run_timers(bus: &mut Bus) {
    let elapsed = sync::resync(bus, TIMERSYNC);

    for (which, &irq) in TIMER_IRQ.iter().enumerate() {
        if bus.timers.run_cpu(which, elapsed) {
            irq::trigger(bus, irq);
        }
    }
}
...
pub fn run_gpu_clocks(bus: &mut Bus, gpu_clock_cycles: ClockCycle) {
    // Only Timer 0 can work on the GPU clock
    let which = 0;

    if bus.timers.run_gpu_clock(which, gpu_clock_cycles) {
        irq::trigger(bus, TIMER_IRQ[which]);
    }
...
pub fn set_in_hsync(bus: &mut Bus, entered_hsync: bool) {
    run_timers(bus);

    bus.timers.set_in_hsync(entered_hsync);

    // Timer 0's counter can optionally be reset to 0 when we enter *or* leave HSync
    let which = 0;

    let sync_mode = bus.timers.sync_mode(which);
    ...
    if needs_reset {
        ...
        bus.timers[which].counter = 0;

        if bus.timers[which].target_match() && bus.timers[which].set_match() {
            irq::trigger(bus, TIMER_IRQ[which]);
        }
    }
    ...
    if entered_hsync {
        ...
        if bus.timers.run_gpu_hsync(which, 1) {
            irq::trigger(bus, TIMER_IRQ[which]);
        }
    }
...
pub fn set_in_vsync(bus: &mut Bus, entered_vsync: bool) {
    run_timers(bus);

    bus.timers.set_in_vsync(entered_vsync);
...
fn predict_next_sync(bus: &mut Bus) {
    ...
    // could wait for a long time, as long as we don't overflow the ClockCycle.
    let default = 0x1_0000;

    let delta = bus.timers.next_irq().unwrap_or(default);

    sync::next_event(bus, TIMERSYNC, delta);
}
...
pub fn store<T: Addressable>(bus: &mut Bus, offset: u32, val: T) {
    ...
    match offset & 0xf {
        0x0 => bus.timers[which].write_counter(val),
        0x4 => {
            bus.timers[which].set_mode(val);
            bus.timers.refresh_sync();
        }
