...
use crate::ps1::psx::graphics::gpu::{DrawMode, State};
    ...
    Shading: ShadingMode,
{
    ...
    let clip = min_x < bus.gpu.clip_x_min
        || max_x > bus.gpu.clip_x_max
        || min_y < bus.gpu.clip_y_min
        || max_y > bus.gpu.clip_y_max;
    ...
    let npixels = if clip {
        ...
        if min_x < bus.gpu.clip_x_min {
            bb_clip_width -= bus.gpu.clip_x_min - min_x;
        }
        if max_x > bus.gpu.clip_x_max {
            bb_clip_width -= max_x - bus.gpu.clip_x_max;
        }
        if min_y < bus.gpu.clip_y_min {
            bb_clip_height -= bus.gpu.clip_y_min - min_y;
        }
        if max_y > bus.gpu.clip_y_max {
            bb_clip_height -= max_y - bus.gpu.clip_y_max;
        }
    ...
    draw_time += if Texture::is_textured() || Shading::is_shaded() {
        ...
        // hits are likely?
        npixels * 2
    } else if Transparency::is_transparent() || bus.gpu.mask_settings.check_mask_bit() {
    ...
    if !bus.gpu.draw_mode.draw_to_display_area() && bus.gpu.display_mode.is_true_interlaced() {
...
fn cmd_handle_poly_tri<Transparency, Texture, Shading>(bus: &mut Bus)
where
    Transparency: TransparencyMode,
    Texture: TextureMode,
    Shading: ShadingMode,
{
    ...
    for (v, coord) in coords.iter_mut().enumerate() {
        ...
        let cmd = bus.gpu.command_pop_to_rasterizer();
        *coord = Position::from_command(cmd);

        // Add the draw offset
        coord.x += bus.gpu.draw_offset_x;
        coord.y += bus.gpu.draw_offset_y;

        if Texture::is_textured() {
            // Pop texture coordinates
            let tex = bus.gpu.command_pop_to_rasterizer();
            if v == 1 {
                // For polygons the 2nd texel contains the new texture page config
                bus.gpu.draw_mode.update_from_poly(tex);
            }
        }
    }

    // Compute the time needed to draw the triangle
    let draw_time = triangle_draw_time::<Transparency, Texture, Shading>(bus, coords);
    bus.gpu.draw_time(64 + 18 + draw_time);
}

fn cmd_handle_poly_quad<Transparency, Texture, Shading>(bus: &mut Bus)
where
    Transparency: TransparencyMode,
    Texture: TextureMode,
    Shading: ShadingMode,
{
    ...
    let draw_time = triangle_draw_time::<Transparency, Texture, Shading>(bus, triangle_1);
    bus.gpu.draw_time(64 + 18 + draw_time);

    // Compute the time needed to draw the second triangle and save it for later
    let draw_time = triangle_draw_time::<Transparency, Texture, Shading>(bus, triangle_2);

    bus.gpu.state = State::InQuad(28 + 18 + draw_time);
}
    ...
    Texture: TextureMode,
{
    ...
    let mut top_left_corner = Position::from_command(bus.gpu.command_pop_to_rasterizer());
    // Add the draw offset
    top_left_corner.x += bus.gpu.draw_offset_x;
    top_left_corner.y += bus.gpu.draw_offset_y;
    ...
    let (mut width, mut height) = match dimensions {
        ...
        None => {
            // Variable dimensions
            let dim = bus.gpu.command_pop_to_rasterizer();
            let w = dim & 0x3ff;
            let h = (dim >> 16) & 0x1ff;
    ...
    if min_x < bus.gpu.clip_x_min {
        width -= bus.gpu.clip_x_min - min_x;
    }
    if max_x > bus.gpu.clip_x_max {
        width -= max_x - bus.gpu.clip_x_max;
    }
    if min_y < bus.gpu.clip_y_min {
        height -= bus.gpu.clip_y_min - min_y;
    }
    if max_y > bus.gpu.clip_y_max {
        height -= max_y - bus.gpu.clip_y_max;
    }
    ...
    if Transparency::is_transparent() || bus.gpu.mask_settings.check_mask_bit() {
        // If we need to read the VRAM for transparency or mask checking we use about 1.5 cycles
        // per pixel.
...
fn cmd_handle_rect_variable<Transparency, Texture>(bus: &mut Bus)
where
    Transparency: TransparencyMode,
    Texture: TextureMode,
{
    let draw_time = rect_draw_time::<Transparency, Texture>(bus, None);
    bus.gpu.draw_time(draw_time);
}

fn cmd_handle_rect_1x1<Transparency, Texture>(bus: &mut Bus)
where
    Transparency: TransparencyMode,
    Texture: TextureMode,
{
    let draw_time = rect_draw_time::<Transparency, Texture>(bus, Some((1, 1)));
    bus.gpu.draw_time(draw_time);
}

fn cmd_handle_rect_8x8<Transparency, Texture>(bus: &mut Bus)
where
    Transparency: TransparencyMode,
    Texture: TextureMode,
{
    let draw_time = rect_draw_time::<Transparency, Texture>(bus, Some((8, 8)));
    bus.gpu.draw_time(draw_time);
}

fn cmd_handle_rect_16x16<Transparency, Texture>(bus: &mut Bus)
where
    Transparency: TransparencyMode,
    Texture: TextureMode,
{
    let draw_time = rect_draw_time::<Transparency, Texture>(bus, Some((16, 16)));
    bus.gpu.draw_time(draw_time);
}
...
fn cmd_handle_line<Transparency, Shading>(bus: &mut Bus)
where
    Transparency: TransparencyMode,
    Shading: ShadingMode,
{
    ...
    bus.gpu.command_pop_to_rasterizer();
    // Start position
    let pos = bus.gpu.command_pop_to_rasterizer();
    ...
    let draw_time = line_draw_time::<Transparency, Shading>(bus, start_pos, end_pos);
    bus.gpu.draw_time(draw_time);
}

fn cmd_handle_polyline<Transparency, Shading>(bus: &mut Bus)
where
    Transparency: TransparencyMode,
    Shading: ShadingMode,
{
    let (opcode, start_pos) = match bus.gpu.state {
    ...
    bus.gpu.state = State::PolyLine(opcode, end_pos);
}
...
fn cmd_vram_copy(bus: &mut Bus) {
    ...
    bus.gpu.draw_time(duration as ClockCycle);
}

fn cmd_vram_store(bus: &mut Bus) {
    // Pop command
    bus.gpu.command_pop_to_rasterizer();
    // Position in VRAM
    bus.gpu.command_pop_to_rasterizer();
    ...
    bus.gpu.state = State::VRamStore(nwords as u32);
}

fn cmd_vram_load(bus: &mut Bus) {
    // Pop command
    bus.gpu.command_pop_to_rasterizer();
    // Position in VRAM
    bus.gpu.command_pop_to_rasterizer();
    ...
    if nwords > 0 {
        ...
        // back
        bus.gpu.rasterizer.flush_command_buffer();
        ...
        bus.gpu.state = State::VRamLoad(None);
    }
}

fn cmd_draw_mode(bus: &mut Bus) {
    let mode = bus.gpu.command_pop_to_rasterizer();

    bus.gpu.draw_mode = DrawMode::new(mode);
}

fn cmd_tex_window(bus: &mut Bus) {
    let tw = bus.gpu.command_pop_to_rasterizer();
    bus.gpu.tex_window.set(tw);
}

fn cmd_clip_top_left(bus: &mut Bus) {
    let clip = bus.gpu.command_pop_to_rasterizer();

    bus.gpu.clip_x_min = (clip & 0x3ff) as i32;
    bus.gpu.clip_y_min = ((clip >> 10) & 0x3ff) as i32;
}

fn cmd_clip_bot_right(bus: &mut Bus) {
    let clip = bus.gpu.command_pop_to_rasterizer();

    bus.gpu.clip_x_max = (clip & 0x3ff) as i32;
    bus.gpu.clip_y_max = ((clip >> 10) & 0x3ff) as i32;
}

fn cmd_draw_offset(bus: &mut Bus) {
    let off = bus.gpu.command_pop_to_rasterizer();

    let off_x = off & 0x7ff;
    let off_y = (off >> 11) & 0x7ff;

    // Sign-extend
    bus.gpu.draw_offset_x = extend_to_i32(off_x, 11);
    bus.gpu.draw_offset_y = extend_to_i32(off_y, 11);
}

fn cmd_mask_settings(bus: &mut Bus) {
    let mask_settings = bus.gpu.command_pop_to_rasterizer() & 0x3f_ffff;

    bus.gpu.mask_settings.set(mask_settings)
}

fn cmd_clear_cache(bus: &mut Bus) {
    // XXX for now we don't implement the GPU cache timings
    bus.gpu.command_pop_to_rasterizer();
}

/// Fill a rectangle with a solid color
fn cmd_fill_rect(bus: &mut Bus) {
    let _color = bus.gpu.command_pop_to_rasterizer();
    let _dst = bus.gpu.command_pop_to_rasterizer();
    let dim = bus.gpu.command_pop_to_rasterizer();
...
fn check_poly_callbacks() {
    ...
    for op in 0x20..=0x3f {
        ...
        dummy_bus.gpu.reset(dummy_bus.gpu.video_standard());

        // Fill the FIFO with a dummy command to make sure the handler consumes the right number of
        // words
        for _ in 0..total_len {
            dummy_bus.gpu.command_fifo.push((op << 24) as u32);
        }

        (cmd.handler)(&mut dummy_bus);

        assert_eq!(dummy_bus.gpu.command_fifo.len(), 0);
    }
}
